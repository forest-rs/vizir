...
use vizir_charts::{
    ...
    PLOT_BACKGROUND, RectMarkSpec, RuleMarkSpec, ScaleBand, ScaleLinearSpec, SectorMarkSpec, Size,
...
fn transforms_demo() -> html::HtmlSection {
    ...
    let axis_bottom_for_scale = axis_bottom.clone();
    let axis_left_for_scale = axis_left.clone();
    ...
    let (_layout, svg) = render_chart(&mut scene, &measurer, &chart, |plot| {
        let x_scale = axis_bottom_for_scale.scale_continuous(plot);
        let y_scale = axis_left_for_scale.scale_continuous(plot);
...
fn aggregate_demo() -> html::HtmlSection {
    ...
    let (_layout, svg) = render_chart(&mut scene, &measurer, &chart, move |plot| {
...
fn stack_demo() -> html::HtmlSection {
    ...
    let (_layout, svg) = render_chart(&mut scene, &measurer, &chart_spec, move |plot| {
...
fn bar_demo() -> html::HtmlSection {
    ...
    let (_layout, svg) = render_chart(&mut scene, &measurer, &chart_spec, move |plot| {
        ...
        // Zero baseline.
        marks.push(
            RuleMarkSpec::horizontal(vizir_core::MarkId::from_raw(0x1F_100), y0, plot.x0, plot.x1)
...
fn area_demo() -> html::HtmlSection {
    ...
    let (_layout, svg) = render_chart(&mut scene, &measurer, &chart_spec, move |plot| {
        ...
        let area = vizir_charts::AreaMarkSpec::new(0x400, table_id, x_col, y_col, x_scale, y_scale)
