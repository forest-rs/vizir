...
Provides versioned `Table`/`Signal` inputs, stable `MarkId` identity, UI-neutral mark metadata, table schema metadata, typed table-data accessors, explicit dependency tracking, and `Enter/Update/Exit` diffs via `Scene::update`.
